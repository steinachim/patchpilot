...
What still cannot be verified *by the app* is the audible recall. The instrument transmits nothing when it loads a preset, and every surface a client can read — the display, the settings block, the preset list — reports the pointer. So a client can confirm that the instrument was asked to load a preset and that the pointer moved, but only a listener can confirm the sound followed.

**Both halves have been confirmed on hardware**, by ear rather than by the app, so the sequence above is measured behaviour and not an inference: selecting a preset changes what the instrument plays and not merely what it displays, and it does so with `MIDI RX Channel` set to `OFF` — the configuration in which the channel-voice path could do nothing at all. That distinction is worth keeping in mind when changing any of this: a settings write alone moves the display and every readable field while the voice engine keeps playing the previous patch, so a regression here looks like success from every surface the app can inspect. Checking it means listening.
...
**Favorited voices — a second undocumented address family at `0x71`**: the instrument keeps a mark against every voice it holds, set from its own front panel (Category Search → the FAVORITE bank), and these are readable over an address family parallel to the `0x0C` voice extension: a dump request at `71 mm 00`, where `mm` is the *same* bank byte a voice dump uses. One small dump per bank answers with **one raw byte per slot**, indexed by zero-based slot — `0` for unmarked, and `1`, `2` or `3` for marked. What distinguishes those three values is not known; all fifteen banks together carry 1,633 marks, and the app treats any non-zero value as "favorited", carries it no further, and shows it nowhere. Two properties of this payload are worth stating because both are easy to get wrong: it is **not MSB-packed**, unlike every voice dump on this wire, so running the unpacker over it yields a shorter array of plausible-looking rubbish rather than an error; and its length is the bank's slot count, so PRE DR answers with 64 bytes, USER DR with 32 and GM DR with exactly 1.

The hazard here is the address itself. **An unmapped middle byte is not merely refused — it puts an *Illegal Bulk Data* message on the instrument's own screen**, in front of whoever is playing it, once per attempt; and `mm = 0x08` is a permanent hole between PRE8 (`0x07`) and GM (`0x09`). So the app drives this walk from its own bank table and never sweeps a range. The family is read-only as far as anything here knows: whether the instrument would accept a bulk dump *to* `0x71` has never been tested, an ill-formed write to this instrument has been observed to hang its MIDI handling until a power cycle, and the app therefore has no writer for it and offers no way to mark a favorite. A bank whose reply is missing or damaged costs only its own marks; the rest of the walk continues.

**Factory voice names are not read at all**: the eleven read-only banks (PRE1–PRE8, GM, PRE DR, GM DR — 1,217 voices) hold data identical on every Motif XS, and reading them costs about seven and a half minutes at ~160 ms per normal voice and a full second per drum kit. The app ships their names instead, transcribed from Yamaha's Data List into `devices/motifxs_factory_voices.json`, and the browser's Factory listing issues no round trips whatsoever. The browser says so on screen, since these are the only names in the app that were not read off the instrument showing them; the hidden debug screen can read one bank live and diff it against the shipped table. Note that the bank-select pairs for these banks are not uniform — PRE1, GM and GM DR all use bank-select LSB `0x00` and are told apart only by the MSB (`0x3F`, `0x00` and `0x7F` respectively), and an unrecognised pair is ignored rather than refused, so a wrong value loads a different bank's voice and reports success.
