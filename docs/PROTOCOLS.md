...
**Never sent**: factory reset (`0x7D`) is named as a constant but never referenced elsewhere in the code — it has no confirmation and no undo. The same applies to every **non-zero** parameter of `0x32`, which puts the synth into a state where it displays 8888 and stops responding properly to its own controls; the safety belongs to the parameter, not the type, so `0x32 0x00` (the preset recall above) is sent and nothing else is. Naming and describing a destructive, otherwise-unlabeled opcode here means nobody building a raw-message tool later sends it by accident, having no way to know what it does.
