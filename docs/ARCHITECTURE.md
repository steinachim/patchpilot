...
`InstrumentViewModel.performConnect` reads `ConnectionPreferences.autoConnectToFirstFound` (Preferences DataStore, default true) on every scan. On, it scans the USB bus first and, if that alone found a recognised instrument, connects to it without running the MIDI scan at all: `mergeCandidates` prefers a USB candidate over a MIDI one for the same physical device, so the MIDI scan (which probes every port with a timeout of up to 600 ms each) could not change the outcome. Off, or where USB alone found nothing recognised, it runs the full two-bus scan and either connects to the first result or, with auto-connect off, stops at `ConnectionState.DeviceSelection` so the user picks explicitly.

## Catalog and adding a device family

Each family is described by one JSON file under `devices/`. `InstrumentRegistry` is a compile-time list of families, each of which loads its own catalog and builds its `Instrument` on an opened transport. The build copies the catalogs into the app's assets and generates the USB attach filter (`res/xml/device_filter.xml`) from their USB ids; [devices/README.md](../devices/README.md) describes both steps and the catalog formats.

Adding a device that speaks an already-implemented family's protocol is a catalog entry. Adding a new protocol is a new package under `devices/` implementing at least `Instrument.browser`, a new catalog file, and one entry in `InstrumentRegistry`.
...
`cache.CachingBrowser` decorates a family's `PresetBrowser` and keeps a completed user listing in `PresetIndexCache`, keyed by instrument identity, firmware version, bank layout and the session's USB device path or MIDI device id, so a layout change invalidates the cache by construction. A physical replug does not reliably change the device path (on the hardware measured, Android hands the same port the same path again), so the ViewModel drops the cached listing on the USB detach broadcast for the session's device (`handleUsbDetach`) instead: once the instrument has left the bus its contents may have changed from its own panel, or a different unit of the same model (which no family can tell apart, since none reads a serial number) may be plugged into the same port. The cache exists because the app rebuilds a USB session on every return to the foreground (`Instrument.rebuildOnResume`); without it a Motif XS would re-read all 416 user voices (about 93 seconds) every time the user switched apps. The cache is owned by the ViewModel, so it outlives the instrument object a reconnect replaces. No family knows it exists.
...
- **A physical detach drops the cached listing; a resume keeps it.** `DeviceLost` is the boundary `PresetIndexCache` honours, and the detach broadcast is the one reliable sign of it (see "Caching"). The `Disconnected → Searching → Opening → Connected` dip of a `forceReconnect()` crosses no such boundary, which is what the cache exists to survive.
- **Backing out of a gate lands on the picker, never on a bare "Not connected".** `ConnectScreen` starts a scan of its own only when it is first shown, so declining the firmware advisory (`declineAdvisory`) or cancelling the unknown-device warning has to bring the next choice with it: both end on `DeviceSelection`, the advisory's path through a teardown and a rescan with the picker forced so auto-connect cannot reopen the same instrument and raise the same advisory again.
- **A scan that a resume would throw away is cancelled on pause.** `MainActivity.onPause` calls `cancelScanOnBackground()`, which stops a running index scan only where `shouldRebuildOnResume` is true, the same answer that makes the resume tear the session down and discard the scan. Letting it run would only keep the instrument busy for nobody; on a Motif XS that is its own display on "dump in progress" for up to 93 s with the phone locked. A Pro-800 over `MidiManager` does not rebuild on resume, so its scan keeps running in the background and is complete when the app returns. An edit, report or regression run is never cancelled this way: it holds `instrumentMutex`, and the resume's teardown waits for it.
...
- **The connection is never closed under an edit.** `onCleared()` takes `instrumentMutex` before closing the instrument; where an edit's `NonCancellable` section still holds it, the close is deferred until that section has finished.
