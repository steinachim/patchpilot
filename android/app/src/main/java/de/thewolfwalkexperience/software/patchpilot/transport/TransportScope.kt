 ...
 *
 * Both families used to build this inline as `CoroutineScope(SupervisorJob())`, which was wrong in
 * two ways that only show up when something goes wrong on the bus.
 *
 ...
 * installed - there was none anywhere in this app - it reaches the platform's default one, which
 * kills the process. That made an ordinary endpoint error a crash. The handler below is the
 * last-resort net for that; the loops themselves are expected to handle their own failures (see
 * [UsbMidiBulkTransport]'s reader), and anything arriving here is a bug worth the error log.
 *
